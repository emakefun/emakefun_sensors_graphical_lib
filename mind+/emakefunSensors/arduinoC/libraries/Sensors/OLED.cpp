...
//等待信号响应
void OLED::I2C_WaitAck(void) //测数据信号的电平
{
  OLED_SCLK_Set();
  OLED_SCLK_Clr();
}
...
//写入一个字节
void OLED::Send_Byte(uint8_t dat)
{
  uint8_t i;
  for(i=0;i<8;i++)
  {
...
//x:0~127
//y:0~63
void OLED::OLED_DrawPoint(uint8_t x,uint8_t y)
{
  uint8_t i,m,n;
  i=y/8;
  m=y%8;
...
//x:0~127
//y:0~63
void OLED::OLED_ClearPoint(uint8_t x,uint8_t y)
{
  uint8_t i,m,n;
  i=y/8;
  m=y%8;
...
//x:0~128
//y:0~64
void OLED::OLED_DrawLine(uint8_t x1,uint8_t y1,uint8_t x2,uint8_t y2)
{
...
//x,y:圆心坐标
//r:圆的半径
void OLED::OLED_DrawCircle(uint8_t x,uint8_t y,uint8_t r)
{
  int a, b,num;
    a = 0;
    b = r;
...
//size:选择字体 12/16/24
//取模方式 逐列式
void OLED::OLED_ShowChar(uint8_t x,uint8_t y,const char chr,uint8_t size1)
{
...
//size1:字体大小 
//*chr:字符串起始地址 
void OLED::OLED_ShowString(uint8_t x,uint8_t y,const char *chr,uint8_t size1)
{
...
//m^n
u32 OLED::OLED_Pow(uint8_t m,uint8_t n)
{
  u32 result=1;
  while(n--)
  {
...
////len :数字的位数
////size:字体大小
void OLED::OLED_ShowNum(uint8_t x,uint8_t y,int num,uint8_t len,uint8_t size1)
{
  uint8_t t,temp;
  for(t=0;t<len;t++)
  {
...
//num:汉字对应的序号
//取模方式 列行式
void OLED::OLED_ShowChinese(uint8_t x,uint8_t y,const uint8_t num,uint8_t size1)
{
...
//配置写入数据的起始位置
void OLED::OLED_WR_BP(uint8_t x,uint8_t y)
{
...
//x1,y1：终点坐标
//BMP[]：要写入的图片数组
void OLED::OLED_ShowPicture(uint8_t x0,uint8_t y0,uint8_t x1,uint8_t y1,const uint8_t BMP[])
{
  int j=0;
  uint8_t t;
  uint8_t x,y;
