...
EMMax72xxPanel::EMMax72xxPanel(byte mosiPin, byte clkPin, byte csPin, byte hDisplays, byte vDisplays) : Adafruit_GFX(hDisplays << 3, vDisplays << 3) {

	EMMax72xxPanel::SPI_CS = csPin;

	byte displays = hDisplays * vDisplays;
	EMMax72xxPanel::hDisplays = hDisplays;
	EMMax72xxPanel::bitmapSize = displays << 3;

	EMMax72xxPanel::bitmap = (byte*)malloc(bitmapSize);
	EMMax72xxPanel::matrixRotation = (byte*)malloc(displays);
	EMMax72xxPanel::matrixPosition = (byte*)malloc(displays);
...
void EMMax72xxPanel::setPosition(byte display, byte x, byte y) {
	matrixPosition[x + hDisplays * y] = display;
}

void EMMax72xxPanel::setRotation(byte display, byte rotation) {
	matrixRotation[display] = rotation;
}

void EMMax72xxPanel::setRotation(uint8_t rotation) {
	Adafruit_GFX::setRotation(rotation);
}

void EMMax72xxPanel::shutdown(boolean b) {
	spiTransfer(OP_SHUTDOWN, b ? 0 : 1);
}

void EMMax72xxPanel::setIntensity(byte intensity) {
	spiTransfer(OP_INTENSITY, intensity);
}

void EMMax72xxPanel::fillScreen(uint16_t color) {
	memset(bitmap, color ? 0xff : 0, bitmapSize);
}

void EMMax72xxPanel::drawPixel(int16_t xx, int16_t yy, uint16_t color) {
...
void EMMax72xxPanel::spiTransfer(byte opcode, byte data) {
...
void EMMax72xxPanel::scrollMessage(String msg,int displayScrollSpeed) {//滚动显示文本
	msg += "";
	int refresh=1;
...
void EMMax72xxPanel::scrollMessage(int num,int displayScrollSpeed) {//滚动显示整数
...
void EMMax72xxPanel::scrollMessage(float num,int displayScrollSpeed) {//滚动显示小数
	String msg = String("") + num;
	msg += "";
	int refresh=1;
	for ( int i = 0 ; i < 6 * msg.length() + WIDTH- 2; i++ ) {
